...
```lisp
(name-char "Tab")       ; → #\Tab    (if supported)
(name-char "Return")    ; → #\Return (if supported)
(name-char "Backspace") ; → #\Backspace (if supported)
```
...
```lisp
(char-name (name-char "Space"))   ; → "Space"
(char-name (name-char "Newline")) ; → "Newline"
