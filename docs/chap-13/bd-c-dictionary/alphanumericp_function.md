...
(my-alphanumericp #\a) ; → T
(my-alphanumericp #\7) ; → 7 (a true value)
(my-alphanumericp #\!) ; → NIL
```
...
```lisp
(every #'alphanumericp "Hello123") ; → T
(every #'alphanumericp "Hello World") ; → NIL
(every #'alphanumericp "no-hyphens") ; → NIL
```
