...
(search "cat" "it's a dog's life") ; → NIL
```
...
```lisp
(search '(c d) '(a b c d e)) ; → 2
(search '(x y) '(a b c)) ; → NIL
```

### Case-insensitive search with :test

```lisp
(search "WORLD" "hello world" :test #'char-equal) ; → 6
```
