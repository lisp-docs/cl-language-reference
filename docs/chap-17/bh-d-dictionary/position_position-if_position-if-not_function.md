...
```lisp
(position 'c '(a b c d e)) ; → 2
(position #\l "hello") ; → 2
(position 99 '(1 2 3)) ; → NIL
```
...
```lisp
(position-if #'evenp '(1 3 5 2 7)) ; → 3
(position-if #'digit-char-p "abc123") ; → 3
```
...
```lisp
(position 3 '((a 1) (b 2) (c 3)) :key #'second) ; → 2

(position-if #'oddp '((1 a) (2 b) (3 c)) :start 1 :key #'car) ; → 2
```
...
```lisp
(position #\o "hello world" :start 5) ; → 7
```
