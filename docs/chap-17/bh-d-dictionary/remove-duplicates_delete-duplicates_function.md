...
```lisp
(remove-duplicates '(a b c b d a e))
=> (C B D A E)
...
```lisp
(remove-duplicates '(a b c b d a e) :from-end t)
=> (A B C D E)
(remove-duplicates "abracadabra" :from-end t)
=> "abrcd"
```
...
```lisp
(remove-duplicates "aAbBcC" :test #'char-equal)
=> "ABC"
(remove-duplicates "aAbBcC" :test #'char-equal :from-end t)
=> "abc"
```
