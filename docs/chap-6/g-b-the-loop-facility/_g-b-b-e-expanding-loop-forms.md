...
The loop prologue contains <GlossaryTerm styled={true} term={"form"}><i>forms</i></GlossaryTerm> that are executed before iteration begins, such as any automatic variable initializations prescribed by the <GlossaryTerm styled={true} term={"variable"}><i>variable</i></GlossaryTerm> clauses, along with any `initially clauses` in the order they appear in the source. 
...
The loop epilogue contains <GlossaryTerm styled={true} term={"form"}><i>forms</i></GlossaryTerm> that are executed after iteration terminates, such as `finally clauses`, if any, along with any implicit return value from an `accumulation clause` or a `termination-test clause`. 
