...
```lisp
(float-radix 1.0)     ; → 2 (binary representation on most systems)
(float-digits 1.0)    ; → 24 (implementation-dependent, typical for single-float)
(float-precision 1.0) ; → 24 (same as digits for normalized floats)
(float-precision 0.0) ; → 0 (zero has no significant digits)
```
...
```lisp
(integer-decode-float 1.0) ; → 8388608, -23, 1 (implementation-dependent)
