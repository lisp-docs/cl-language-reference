...
```lisp
(typep 0 '(unsigned-byte 8))     ; → T
(typep 255 '(unsigned-byte 8))   ; → T
(typep 256 '(unsigned-byte 8))   ; → NIL
(typep -1 '(unsigned-byte 8))    ; → NIL  (no negative values)
```
...
```lisp
(subtypep 'bit '(unsigned-byte 1))          ; → T, T
(subtypep '(unsigned-byte 1) 'bit)          ; → T, T
(subtypep '(unsigned-byte 8) '(signed-byte 9)) ; → T, T
(subtypep '(unsigned-byte 8) '(unsigned-byte 16)) ; → T, T
```
