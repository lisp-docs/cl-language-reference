...
(funcall (symbol-function 'square) 5) ; → 25
```
...
(funcall (symbol-function 'car) '(a b c)) ; → A
(funcall (symbol-function '+) 1 2 3) ; → 6
```
...
(safe-symbol-function 'car) ; → #<FUNCTION CAR>
(safe-symbol-function 'not-yet-defined) ; → NIL
(safe-symbol-function 'defun) ; → NIL  (defun is a macro)
```
