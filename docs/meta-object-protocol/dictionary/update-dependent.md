...
In each case, [map-dependents](/meta-object-protocol/dictionary/map-dependents) is used to call `update-dependent` on each of the dependents. So, for example, the update of a generic function's dependents when a method is added could be performed by the following code:
